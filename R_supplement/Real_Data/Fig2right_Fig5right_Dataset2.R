...
(round((param_est[g.ind,1])-(variat[g.ind]),5)==max(round((param_est[g.ind,1])-(variat[g.ind]),5)))
######################################################################
######################################################################
######################################################################
######################################################################
...
data(nature2008);   GroupVar=c("sampleType");   objectName<-"nature2008"
LoadFileName<-paste0(objectName,"_Single.RData")
load(LoadFileName)
cts <- qPCRdata
...
boxplot(boxesHDAC, xlab="", ylab=ylable, ylim=c(-10,10),xaxt="n", cex.lab=1.5)
abline(h=0)
# x-axis lable 
